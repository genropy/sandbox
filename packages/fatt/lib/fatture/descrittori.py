...
class FattureManager(object):
    ...
    def creaDescrittoreFattura(self, fattura_id): # fattura_corrente
        ...
        fattSrc = FatturaStruttura() # creo lo scheletro della Fattura in formato Structure
        fatt = fattSrc.fattura(cliente_id=fattura_corrente['cliente_id'], data=self.db.workdate) # inserisco i dati della "testata" e mi posiziono, per così dire, a questo livello
        
        # adesso localizza le righe di questa fattura_corrente perché dobbiamo caricarle nella Structure
    ...
    def scriviFatturaDaDescrittore(self, fattSrc):
        fattSrc.validate()

        self.tblfattura.insert(fattSrc.getAttr('fattura'))
        
        for riga_node in fattSrc['fattura']: # mmm ogni riga è un BagNode
            riga_record = riga_node.attr
            riga_record['fattura_id'] = fattSrc.getAttr('fattura', 'id')
