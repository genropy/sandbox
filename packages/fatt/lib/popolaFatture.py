...
class Populator(object):
    ...
    def __init__(self,app):
        self.app = app
        self.db = db = app.db
        self.tbl_fattura = db.table('fatt.fattura')
        self.tbl_fattura_riga = db.table('fatt.fattura_riga')
        self.tbl_cliente = db.table('fatt.cliente')
        self.tbl_prodotto = db.table('fatt.prodotto')
        data_ultima_fattura = self.tbl_fattura.query('max(data) as max_data').fetch()[0]['max_data']
        if data_ultima_fattura:
            anno, mese, giorno =data_ultima_fattura.split('-')
            data_ultima_fattura = date(int(anno),int(mese),int(giorno))
        self.prodotti = self.tbl_prodotto.query('*,@tipo_iva_codice.aliquota as aliquota_prodotto').fetch()
        self.clienti = self.tbl_cliente.query().fetch()
