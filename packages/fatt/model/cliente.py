...
class Table(object):
    ...
    def config_db(self, pkg):
        ...
        tbl.column('provincia',size='2',name_long='!![it]Provincia', name_short='Pr.').relation(
                                                    'glbl.provincia.sigla', relation_name='clienti', mode='foreignkey',onDelete='raise')
        tbl.column('comune_id',size='22' ,group='_',name_long='!![it]Comune').relation(
                                                    'glbl.comune.id',relation_name='clienti',mode='foreignkey',onDelete='raise')
        tbl.column('cliente_tipo_codice',size=':5',name_long='!![it]Tipo cliente',name_short='!![it]Tipo').relation(
                                                    'cliente_tipo.codice',relation_name='clienti',mode='foreignkey',onDelete='raise')
        tbl.column('pagamento_tipo_codice',size=':5',name_long='!![it]Tipo pagamento',name_short='!![it]Tipo pagamento').relation(
                                                    'pagamento_tipo.codice',relation_name='clienti',mode='foreignkey',onDelete='raise')
        ...
        tbl.aliasColumn('regione_sigla', relation_path='@provincia.regione', name_long='Sigla regione').relation(
                                                    'glbl.regione.sigla', mode='foreignkey')
        tbl.aliasColumn('regione_nome', relation_path='@provincia.@regione.nome', many_name='Clienti', name_long='Regione')
