...
class Form(BaseComponent):
    ...
    def th_form(self, form):
        ...
        fb.div(hidden='^.startup_data_ts').button('Inizializza dati',
                                                    hidden='^.active_dbstore?=!#v').dataRpc(
                                                        self.startupDataInWorkspace,
                                                        timeout=0,
                                                        pkey='=#FORM.record.id',
        ...
        #                ).thIframe(self.iframe_datiSocieta,dbstore='^#FORM.record.dbstore')


    @public_method
    def startupDataInWorkspace(self,pkey=None):
        tblobj = self.db.table('shop.shop')
        tblobj.multidb_setStartupData(pkey)
